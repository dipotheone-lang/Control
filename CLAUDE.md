...
**Charter version:** 4.15 — 31-Aug-2026 — decision D-62 (the register-gap requests are sent by the run itself, to internal holders only, weekly, self-terminating). Prior: 4.14 — 31-Aug-2026 — decision D-61 (a discovery scan ran under a scope that forbids one; output deleted, refusal now enforced in the discovery commands). Prior: 4.13 — 31-Aug-2026 — decision D-60 (backup destination moved to OneDrive; the E: reasoning withdrawn as false after measurement). Prior: 4.12 — 30-Aug-2026 — decisions D-58 (Outlook carries the class 1 alerts; Graph no longer waited on, with the schedule risk accepted and mitigated by retry) and D-59 (payroll split, withholding lead, PDPL anchor, backup destination). Prior: 4.11 — decision D-15 (statutory-only operation: class 1 alone, no mailbox read, on the evidence that no mailbox scan had ever completed and 130 of 314 contract-folder documents were unreadable). Prior: 4.10 — 17-Aug-2026 — decision D-14 (D-05 extended to OCR of confidential contracts, text never retained). Prior: 4.9 — decisions D-07 (O-05 closed on Option C, extended mailbox scope, gated on O-07/O-08/O-10), D-08 (interim Outlook COM transport, Graph required before Phase 2), D-09 (continuity CC retained, reviewed 16-Nov-2026), D-10 (CEO flag budget 10/week), D-11 (encrypted backup to the company tenant) D-12 (draft release, COO deputising during registered CEO absence) and D-13 (report distribution narrowed for Phase 2); O-01, O-09 and O-11 also closed (Appendix B). Prior: 4.4 — D-05 narrow date-extraction exception to D-01 for the class 2 registers; 4.3 — second-round review findings V1–V15 (`docs/CHARTER-REVIEW-v4.2.md`; traceability in Appendix C)
...
**The CEO's own absence is registered the same way.** During registered CEO absence the COO deputises for dispute adjudication, time-critical draft approvals, and **the release of pending drafts** (decision D-12, 16-Aug-2026 — otherwise class 3 escalations stop moving whenever the CEO travels); everything else queues until return. The deputy path opens from the absence register, never from the deputy. The external gate does not open for a deputy, and every deputised approval is logged as such.
...
Recipients per `distribution.yaml`. **Phase 2: CEO and COO only (decision D-13);** widening to CEO, COO, Acting CFO and Ahmed Hassan once the Phase 2 gate is passed. `.xlsx` export plus bilingual summary. Always drafted for approval.
...
**Prohibited, absolutely:** opening the document body · OCR *(except under D-14, for the D-05 purpose only, with the OCR text never retained)* · text extraction · figure extraction · posting any value to a register · quoting any content in any reply or report · retaining the file beyond the mailbox itself · passing contents to any model or external service.
...
**Scope (D-15).** Every row above runs at `OPERATING_SCOPE=FULL` — the charter as written. One further legal state exists outside the phase sequence:

| Scope | Level | RUN_MODE | LEARNING_MODE |
|---|---|---|---|
| STATUTORY_ONLY | 2 | SUPERVISED | OBSERVE |

Class 1 alone, no mailbox read. It is not a phase and does not progress to one: widening means closing the §12 pre-conditions and returning to the sequence at the row the evidence supports. An unrecognised scope halts rather than defaulting, because the default would be the wider one (§5.6).
...
| D-08 | 16-Aug-2026 | **Transport route: Outlook COM for Phase 0, Graph before Phase 2.** §5.1 is unchanged and Graph remains the specified route; this records an interim position, not an amendment. Outlook COM is permitted in `DISCOVERY` and `DRY_RUN` only, and **refused at startup in `SUPERVISED` and `LIVE`** (`config/transport.yaml`). Reasons, both operational: Phase 2 sends on a schedule, and a transport requiring a powered laptop with Outlook open cannot hold one — a missed class 1 alert is this charter's most expensive failure; and Outlook COM sees whatever the Windows profile sees, which is not the set D-07 authorises. Graph's Application Access Policy is enforced by Exchange against a scoped identity, and a permission the system grants itself is not a control | §5.1, §16 | **Never** |
| D-09 | 16-Aug-2026 | **Continuity CC retained, reviewed 16-Nov-2026.** `contact.ubcsis@gmail.com` stays as the D-04 exception rather than being replaced now. What the deferral accepts, recorded so the review has something to weigh: roughly 10,000 messages of company correspondence sit in a consumer account outside company control, every outbound message adds to it, and its status is an open question for counsel (`PDPL-BASIS.md` §7). The standing replacement recommendation is **deferred, not withdrawn**, and Control chases the review date on the same mechanism as D-06 | §3.1, §10, `continuity.yaml` | **Never** |
| D-10 | 16-Aug-2026 | **CEO anomaly-flag budget: 10 per week.** About two a working day — chosen as the number the CEO will still open on the eleventh, because a channel that gets skimmed is a failed control whatever its thresholds say (§13.3). Over-budget flags are **reported as suppressed, never silently dropped**. A Tier A tightening that pushes volume past the budget without producing a confirmed-useful flag auto-rolls back (§14.5). Reviewed after 30 days of Phase 2 volume | §13.3, §14.5, `materiality.yaml` | **Never** |
| D-11 | 16-Aug-2026 | **Backup destination: the company Microsoft 365 tenant** (OneDrive/SharePoint), encrypted, daily, whole of `CONTROL_ROOT`. Reasoning: the mail already lives in that tenant, so this adds no new processor under §12.2 and no new NDA exposure — while an external drive in the same office does not survive the event that takes the laptop. Binding: **the key never lives in a file or this repository**, and a missing key halts rather than falling back to plaintext; the destination path is set on the running machine, never guessed in the repository; and §13.3 continuity reports backup age **and** last successful restore test, because an untested backup is a hope | §5.2, §13.3, `backup.yaml` | **Never** |
| D-12 | 16-Aug-2026 | **Draft release: CEO, with the COO deputising during registered absence.** §10 keeps L3 escalations and management reports at DRAFT permanently, so somebody must release them forever; without a deputy path, class 3 escalations stop moving whenever the CEO travels. This extends the §3.3 CEO-absence rule to draft release rather than creating a new authority. Binding: the deputy path opens from the **absence register**, never from the deputy — an unregistered absence keeps drafts pending; every deputised release is logged with `deputised: true` and whom it was for; §10 authentication is unchanged (in-thread, draft ID quoted, authenticated sender); and **the external gate does not open for a deputy**, or for anyone | §10, §3.3 | **Never** |
| D-13 | 16-Aug-2026 | **Management report distribution narrowed to CEO and COO for Phase 2**, widening to the §11 default (adding the Acting CFO and Ahmed Hassan) once the Phase 2 gate is passed. Reasoning: during the 30 supervised days the report carries Control's own false positives — that is what the phase is for — and §13.1 warns a system that wrongly returns correct work loses authority permanently on one impression. Widening later is easy; unwinding a bad first impression across four people is not. **This is a narrowing by phase, not a private pilot:** §12.4 still requires the usage policy circulated to and acknowledged by everyone before the first live reminder, so the whole company knows the system exists, what it reads, and how to dispute it. Control never advances the phase; it follows the §15 promotion, which is a CEO decision | §11, §13.1, `distribution.yaml` | **Never** |
| D-14 | 17-Aug-2026 | **D-05 extended to permit OCR of client-confidential contracts, for the D-05 purpose only.** §12.1.2 lists OCR under *prohibited, absolutely*; D-05 permits extracting dates and term durations but names no method. The two collide on a scanned NDA contract, and the collision is not theoretical: the first live Stage C run found 47% of legal documents are photographs of text, 81% for supplier legal documents, so the guarantee expiries and forfeitable claim windows D-05 exists to catch are precisely the ones no text layer reaches. All D-05 conditions carry over unchanged — local processing only, nothing to any model or external service, no clause text stored, quoted or reproduced, everything else metadata-only. **One condition is added:** for a client-confidential document the OCR text buffer is **never retained** — it passes to term extraction transiently and the stored result keeps only the confidence and the document reference. Retaining it would place the full body of an NDA contract in a structure the report layer can reach, which is the leak D-05's redaction-at-capture rule exists to prevent, arriving by a different door | §12.1.2, §5.5, D-05 | **Never** |
...
| D-15 | 30-Aug-2026 | **Statutory-only operation.** Control operates on class 1 statutory obligations alone and **reads no mailbox**: deadlines computed from `statutory-calendar.yaml`, alerts to the named owner and the CEO on the §2.1 schedule, nothing else. `OPERATING_SCOPE=STATUTORY_ONLY` with `RUN_MODE=SUPERVISED` becomes a legal state (§16), outside the phase sequence rather than a step in it. **Evidence:** the status page of 30-Aug-2026 — no mailbox scan had ever completed on the operating machine (absent, not quiet), 130 of 314 documents in the contract folders were unreadable even with OCR at a median confidence of 71.4, and nothing in §12 was circulated (usage policy 0 of 11, IWR 0 of 11, PDPL notification not issued). The monitoring half therefore carried every governance cost in §12 while being unproven, and the class 2 registers could not be populated from the archive at all. **Given up:** report chasing, external SLA watchdog, verdicts, S1–S4 anomaly and fraud signals, commercial registers, learning, and management reporting beyond the statutory horizon. **The §3.2 segregation-of-duties exposure is unchanged** — narrowing the software does not narrow it, and it stands in the discovery record. **Unchanged and binding:** §1 in full; §10's external gate; D-08's requirement that Graph carries anything sent on a schedule; §2.1 advisor verification, so **O-03 becomes the only thing between here and operation**; §5.2, §13.3, §5.6. **Not required, and the reasoning is recorded as a basis rather than a conclusion:** no usage policy, PDPL notification or IWR amendment, because §12.4 governs a system measuring named individuals' process compliance and §12.2 governs mailbox-derived personal data, and this scope does neither. **That reasoning is to be put to counsel, not assumed**; O-06, O-07, O-08 and O-10 remain open against any future widening. **Reversible:** nothing is deleted — the evaluation engine, watchdog, registers and learning layer remain under test, and widening means closing the §12 pre-conditions and moving the scope back. *Decided by Ahmed Diab; entered into this file by Control on his express instruction, §17 otherwise reserving every edit to him.* | §0, §16, §12, O-03 | **Never** |

| D-58 | 30-Aug-2026 | **Outlook carries the class 1 alerts; Graph is no longer waited on.** D-08 refused Outlook COM in `SUPERVISED` and `LIVE` for two reasons, and this decision separates them. **The first is gone, not accepted:** Outlook sees whatever the Windows profile sees rather than the set D-07 authorises — which cannot happen under `OPERATING_SCOPE=STATUTORY_ONLY`, where nothing is fetched, classified or evaluated. **The second is accepted with its cost stated:** a transport needing a powered laptop with Outlook running cannot hold a class 1 schedule, so on the day a filing falls due with the machine asleep, nobody is told. That is the failure this charter exists to prevent, and it is chosen knowingly — the alternative on this machine was **no delivery at all**, not Graph, and the tenant was never provisioned. **Bounded:** the permission is conditional on the scope, not the run mode. `OPERATING_SCOPE=FULL` in `SUPERVISED` still halts on the route exactly as D-08 wrote it, so widening the scope cannot quietly widen the route. **Mitigated, because the risk is real:** an alert that cannot leave is written `UNDELIVERED`, never marked sent, and **retried on the next run** — missing T−7 does not silence T−3, T−1 and the day itself; a repeat failure on the same alert is reported separately, because one miss is a closed laptop and a repeat is the transport. **Unchanged:** §10's external gate — nothing leaves for an address outside ubcsis.com, by any route, in any mode. *Raised by Control with the risk stated, decided by Ahmed Diab against that advice, and entered here on his express instruction (§17).* | §5.1, §16, D-08, `transport.yaml` | **Never** |
| D-59 | 30-Aug-2026 | **Four register decisions taken to make the statutory scope operable.** (a) **Payroll tax split** into a monthly remittance and a quarterly reconciliation return. The register carried its own doubt — if the year holds twelve remittances rather than four, one quarterly row hides eleven obligations — and the archive showed twelve. Both rows are `ceo_stated` and unverified; a wrong monthly date fires twelve wrong alerts a year, so O-03 takes these first. (b) **Withholding tax gains a 5-working-day operative lead**, matching VAT: the statutory date stays the anchor, so the lead can only make Control early. (c) **PDPL regulations watch anchored at 1 October, quarterly** — a scheduling choice, not a legal date; the subject remains counsel's and the row stays unverified. (d) **Backup destination set to the company drive**, `E:\UBCSIS Co Date Jan 2026\Control-Backup`. It survives the laptop, which a folder in the user profile would not; it does **not** survive the office, and it backs up nothing on the days E: is unplugged — so §13.3's backup-age line is the control that matters, not the destination. Class 1 coverage moves from 4 of 12 to 7 of 13. *Decided by Ahmed Diab; entered here by Control on his express instruction (§17).* | §2.1, §5.2, D-11 | **Never** |

| D-60 | 31-Aug-2026 | **Backup destination moved from the E: drive to OneDrive, and the E: reasoning is withdrawn as false.** D-59 set the destination to `E:\UBCSIS Co Date Jan 2026\Control-Backup` on the stated reasoning that *"it survives the laptop, which a folder in the user profile would not"*. **Measured on 31-Aug-2026: C: and E: are both partitions of DiskNumber 0** — one physical disk — so that backup survived none of the three events D-11 exists for (disk failure, theft, loss). It protected against accidental deletion of `CONTROL_ROOT` and nothing else. The claim was mine, entered on the CEO's instruction without checking, and it is corrected here rather than left standing. **What replaces it is not what D-11 asked for either:** this machine carries a **personal** OneDrive, not the company Microsoft 365 tenant — the folder is plain `OneDrive` and `OneDriveCommercial` is unset. D-11 named the company tenant precisely because the mail already lives there, adding no new processor under §12.2 and no new NDA exposure; a personal account adds both. **Accepted meanwhile because the archive is encrypted before it is written** (Fernet, key in Windows Credential Manager, never in a file or this repository), so what syncs is ciphertext — a far smaller exposure than readable company records, and it is off the machine, which E: never was. **The condition that makes it a backup at all:** the key is on the laptop, so if the laptop dies and the key was never escrowed elsewhere the synced archives are unrecoverable by anyone, including the CEO. **To be replaced** when OneDrive is provisioned for a ubcsis.com account, which returns the destination to D-11 as written. §13.3's backup-age line and the restore test remain the controls that matter. *Decided by Ahmed Diab; entered here by Control on his express instruction (§17).* | §5.2, §13.3, D-11, D-59 | **Never** |

| D-61 | 31-Aug-2026 | **A discovery scan ran under a scope that forbids one; the output is deleted and the refusal is now enforced.** `phase0` was run on the operating machine while `OPERATING_SCOPE=STATUTORY_ONLY` and scanned **fourteen mailboxes** — including `ahmed@ubcsis.com`, a named individual's mailbox excluded from D-07 by name; `contact.ubcsis@gmail.com`, an external consumer account; and four belonging to a different company (`axionegypt.com`). **This was a defect, not a decision.** D-15's entire basis for operating without the §12 pre-conditions is that no mailbox is read. The refusal existed and was tested, and had been wired into `cycle` and into transport construction — not into discovery, which is the command that opens everything. *Raised by Control against itself.* **What was and was not processed:** metadata only — senders, subjects, timestamps, counts — because Phase 0 is built not to open message bodies, and that constraint held. It is still processing personal data under §12.2 with no documented lawful basis and nobody notified (O-07, O-08 unchanged and still open). **Deleted on the CEO's instruction**, through `control purge --discovery`, which logs the deletion to the hash-chained audit log with the reason and the person who ordered it — §12.5 requires deletion to be deliberate and logged, and a `Remove-Item` leaves the same evidence as data never collected. **The encrypted backup archive written earlier that day still contained a copy**; the purge names any such archive rather than leaving it to be discovered, and completing the deletion means a fresh backup first, then removing the old archive — in that order, because §13.3 counts backup age as a control. **Now enforced:** `phase0` and `outlook-scan` call `assert_scope_permits` before the Outlook namespace is touched at all — not "read nothing", do not connect, since opening the mailbox is the processing §12.2 governs. A test fails if the namespace is reached. Under `OPERATING_SCOPE=FULL` both commands run exactly as before. *Defect found and reported by Control; deletion ordered by Ahmed Diab and entered here on his express instruction (§17).* | §12.2, §12.4, §12.5, §1.9, D-15 | **Never** |

| D-62 | 31-Aug-2026 | **The register-gap requests are sent by the run itself.** Before this, the daily run computed which class 1 rules were waiting on a date somebody in the company already holds, wrote a ready bilingual request per holder to `reports/statutory-ask-*.txt` — and stopped, leaving the CEO to be the mail merge. The CEO's instruction: *"I built the entire system to send automatic emails, not to do it myself."* The `REGISTER_GAP_REQUEST` kind is added to the §10 gate table with the same dispositions as a class 3 reminder — internal, non-committal, chasing a class 1 input — sending in `SUPERVISED` and `LIVE`, drafting in `DRY_RUN`. **Bounds, all enforced where the message is built rather than assumed downstream:** recipients and CC must be `ubcsis.com` addresses — an external holder is refused and reported, with the block left on the ask page to forward by hand, because this decision changed who *sends*, not who may *receive*, and the §10 external gate is untouched; each question asks **once per week**, not daily — the dedupe key carries the ISO week — because a daily nag teaches people to filter the sender, which silences the class 1 alerts travelling under the same name; the requests **self-terminate** the moment the register row gains a usable date; a row whose holder is not recorded in `answer_held_by` produces no message — a name in prose is not a routable address, and the gap is reported instead (§1.1). The ask page remains, reworded as the record of what goes out. **What this does not cover:** the tax advisor (external — the request for O-03 verification still travels person-to-person) and any statutory decision — §14.2 Tier C is unchanged; the system asks for dates, it never sets one. *Instructed by Ahmed Diab; entered here by Control on his express instruction (§17).* | §10, §2.1, D-15, D-58 | **Never** |
